...
# %matplotlib auto --> shows plots in another window
# %matplotlib inline --> shows plots in the python console again
...
vol_in_mtp = 0.2 #volume in mtp ml
dil_fac = 1 #dilution factor
d_thick = 0.625 #d[cm] Schicktdicke
ext_koeff = 10 #Extinktion koefficient [mM-1*cm-1]
vol_of_samp = 0.01 #ml volume of sample
enz_konz = 0.003125 #mg/ml enzyme used 
output_name = 'results_Impranil_50°C_ocemut_1%_50nM_N3.xlsx'


data = pd.read_excel('../RawData/02052023 pnpH + Impranil alle + mut 0,03125.xlsx',sheet_name='Impranil 50°C ocemut')
...
for name in data.keys()[2:]:
    ...
    plt.figure()
    plt.plot(t,y,'o-')
    plt.plot([t[i_start],t[i_start]],[-1,0],'k:') #change yellow numbers to change left dashed line
    plt.plot([t[i_start+n_points-1],t[i_start+n_points-1]],[-1,0],'k:') #change yellow numbers to change right dashed line
    plt.title('%s, slope: %3.3f, R2: %3.3f' % (name,slope,Rsquared))
    plt.xlabel('Time [min]')
...
for enz in slopes_enzyme.keys():
    ...
    result = slope / enz_konz #Für pNP assay ((slope * vol_in_mtp * dil_fac) / (d_thick * ext_koeff * vol_of_samp)) / enz_konz
